...
>
  ...
  <div
    ...
    style:background-image="radial-gradient(rgba(239,68,68,0.12) {Math.max(zoom * 0.7, 0.7)}px, transparent 0)"
    style:background-size="{24 * zoom}px {24 * zoom}px"
    style:background-position="{-zoom * center[0]}px {-zoom * center[1]}px"
  />

  <div class="py-2 px-3 rounded border border-zinc-800/30 bg-zinc-950/50 backdrop-blur-sm w-fit">
    {#if exitReason !== null}
      <div class="flex items-center gap-2 font-mono text-xs"><span class="inline-block w-2 h-2 rounded-full bg-red-500"></span><span class="text-red-400 tracking-wider uppercase">{exitReason}</span></div>
    {:else if connected}
      <div class="flex items-center">
        <div class="flex items-center gap-2 font-mono text-xs"><span class="inline-block w-2 h-2 rounded-full bg-green-500 animate-pulse"></span><span class="text-green-400 tracking-wider uppercase">LINK ESTABLISHED</span></div>
        {#if userId && hasWriteAccess === false}
          <div
            class="bg-amber-900/30 text-amber-300 border border-amber-500/20 px-2 py-0.5 rounded-sm ml-3 inline-flex items-center gap-1.5 font-mono text-[10px] tracking-wider"
          >
            <EyeIcon size="14" />
            <span>OBSERVER</span>
          </div>
    ...
    {:else}
      <div class="flex items-center gap-2 font-mono text-xs"><span class="inline-block w-2 h-2 rounded-full bg-amber-500 animate-pulse"></span><span class="text-amber-400 tracking-wider uppercase">ESTABLISHING LINK...</span></div>
    {/if}
