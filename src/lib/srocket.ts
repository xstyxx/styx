/**
 * @file Internal library for styx, providing real-time communication.
 *
 * The contents of this file are technically general, not styx-specific, but it
 * is not open-sourced as its own library because it's not ready for that.
 */

import { Encoder, Decoder } from "cbor-x";

const cborEncoder = new Encoder({ tagUint8Array: false, useRecords: false, mapsAsObjects: true });
const cborDecoder = new Decoder({ mapsAsObjects: true });
const encode = (msg: any) => cborEncoder.encode(msg);
const decode = (buf: Uint8Array) => cborDecoder.decode(buf);
