...
use http::StatusCode;
use styx::encrypt::Encrypt;
use styx_core::proto::styx_service_client::StyxServiceClient;
use styx_core::{Sid, Uid};
use styx_server::{
    state::ServerState,
...
impl TestServer {
    ...
    pub async fn grpc_client(&self) -> StyxServiceClient<Channel> {
        StyxServiceClient::connect(self.endpoint()).await.unwrap()
    }
