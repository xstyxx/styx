...
use sha2::Sha256;
use styx_core::rand_alphanumeric;
use tokio::time;
...
const DISCONNECTED_SESSION_EXPIRY: Duration = Duration::from_secs(7 * 24 * 3600);
...
impl ServerState {
    ...
    /// Connect to a session by name from the `styx` client, which provides the
